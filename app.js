...
// -----------------------
const MIN_GAP = 8;     // must see 8 other cards before seeing the same one again
let recentQueue = [];  // holds last MIN_GAP spanish strings shown

function recordShown(word) {
  if (!word) return;
  recentQueue.push(word.spanish);
  if (recentQueue.length > MIN_GAP) recentQueue.shift();
}

function excludeRecent(cards) {
  // If the pool is too small, we can't enforce MIN_GAP.
  if (cards.length <= MIN_GAP) return cards;

  const recentSet = new Set(recentQueue);
  const filtered = cards.filter(w => !recentSet.has(w.spanish));

  // If filtering removes everything, fall back to the original cards.
...
function selectNextCard() {
  ...
  // Apply anti-repeat filter to each bucket
  newCards = excludeRecent(newCards);
  learningCards = excludeRecent(learningCards);
  masteredCards = excludeRecent(masteredCards);

  // Priority: 40% new, 40% learning, 20% mastered (tweak as you like)
  const r = Math.random();

  if (r < 0.4 && newCards.length > 0) {
    return pickRandom(newCards);
  } else if (r < 0.8 && learningCards.length > 0) {
    return pickRandom(learningCards);
  } else if (masteredCards.length > 0) {
    return pickRandom(masteredCards);
  }

  // Fallback
  const pool = excludeRecent(availableCards);
...
function showCard(word) {
  ...
  // Track shown cards to prevent repeats
  recordShown(word);
}
...
function resetProgress() {
  if (confirm('Are you sure you want to reset all your progress? This cannot be undone.')) {
    ...
    // Also reset anti-repeat history
    recentQueue = [];
...
document.querySelectorAll('.filter-btn').forEach(button => {
  button.addEventListener('click', function() {
    ...
    // Reset recent history when switching filters (optional but helps)
    recentQueue = [];
